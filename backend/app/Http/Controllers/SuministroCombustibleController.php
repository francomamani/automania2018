...
class SuministroCombustibleController extends Controller
{
    ...
    public function store()
    {
        ...
        return response()->json([
            'mensaje' => 'Suministro de combustible registrado exitosamente. ¿Desea volver al listado de suministro de combustibles??',
            'has_action' => true,
        ], 201);
    }
    ...
    public function filtrar()
    {
        ...
        switch ($status) {
            case 'placa-asc':
                $response = SuministroCombustible::orderBy('vehiculo_id')->with('vehiculo_id')->get();
                break;
            case 'placa-desc':
                $response = SuministroCombustible::orderByDesc('vehiculo_id')->with('vehiculo_id')->get();
                break;
            case 'combustible-asc':
                $response = SuministroCombustible::orderBy('combustible')->with('kilometrajes')->get();
                break;
            case 'combustible-desc':
                $response = SuministroCombustible::orderByDesc('combustible')->with('kilometrajes')->get();
                break;
        }
